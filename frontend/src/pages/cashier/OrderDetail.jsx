...
const OrderDetail = () => {
  ...
  const getNextStatus = (currentStatus) => {
    ...
    switch (currentStatus) {
      ...
      case 'Selesai Produksi':
        return 'Siap Kirim';
      default:
        return null; // Tidak bisa mengubah status lain
    }
  };
  ...
  const handlePayment = async (paymentData) => {
    try {
      ...
      // Untuk pesanan offline/manual, gunakan pendekatan langsung
      if (order.isOfflineOrder) {
        ...
        if (paymentData.isRemainingPayment) {
          ...
          try {
            ...
            console.log('Payment method type:', typeof paymentRequestData.method);
            console.log('Amount type:', typeof paymentRequestData.amount);
            console.log('OrderId type:', typeof paymentRequestData.orderId);
        ...
        } else if (order.paymentDetails.downPayment && order.paymentDetails.downPayment.status !== "paid") {
          // Untuk pembayaran DP (backward compatibility - jika isDownPayment tidak ada)
          updateData = {
            status: order.status, // Tetap status yang sama
            notes: `Pembayaran DP sebesar ${formatCurrency(order.paymentDetails.downPayment.amount)} dengan metode ${paymentData.method || 'cash'}`
          };
        ...
        if (updateStatus) {
          // Hanya update catatan status pesanan, bukan status pembayaran
        ...
        // Untuk pesanan online, coba pendekatan alternatif
        showNotification('Pembayaran untuk pesanan online masih dalam pengembangan', 'info');
        setShowPaymentForm(false);
      }
    } catch (error) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {nextStatus && ['Pesanan Diterima', 'Siap Kirim'].includes(nextStatus) && (
            <Button 
                      ...
                      <div className="mt-4 space-y-3">
                        ...
                        {item.customDesign && item.customDesign.notes && (
                          <div className="bg-blue-50 rounded-md p-3">
                            <div className="text-xs text-gray-500 font-medium mb-1">Catatan Desain Custom</div>
            ...
            <div className="space-y-3">
               ...
               order.paymentDetails.remainingPayment.status === 'pending' && (
                <div className="mt-4">
                  <Button
                    ...
                    onClick={handleProcessRemainingPayment}
               ...
               !(order.isOfflineOrder && 
                ...
                <div className="mt-4">
                  <Button
                    ...
                    onClick={handleProcessFullPayment}
                    className="w-full"
                  />
                </div>
              )}
              ...
              {order.isOfflineOrder && 
               ...
               order.paymentDetails.downPayment.status === 'pending' && (
                <div className="mt-4">
                  <Button
                    label="Proses Pembayaran DP"
                    onClick={handleProcessDownPayment}
